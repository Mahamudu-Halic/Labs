...
const AddOns = ({addons, handleAddons}: {
    addons: ServiceType[];
    ...
    const addon = addonsList.map((item: AddOnsListType) => (
        <div key={item.key} className="add-on__item">
            <input type="checkbox" name="" id={item.title} onChange={() => handleAddons(item)}
                   checked={!!addons.find((addOn: ServiceType) => addOn.title === item.title)}/>
            <label htmlFor={item.title}>
                ...
                <p className="add-on__item-price">
                    +${timeFrame === "yearly" ? (item.price * 10) + `/yr` : item.price + `/mo`}
                </p>
            </label>
        </div>
    ))
